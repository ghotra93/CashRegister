...
| AC-025 | T-005, T-011, T-014 | 5 | `DivisorEndpointsTests.Put_ValidDivisor_ReturnsItAndGetReflectsIt`<br>`DivisorEndpointsTests.Put_NewDivisor_AppliesToFilesProcessedAfterwards`<br>`ChangeCalculatorTests.Calculate_DivisorChanged_AppliesToNextCalculation`<br>`InMemoryDivisorSettingsTests.Change_PositiveInteger_ChangesCurrent`<br>… +1 more |
| AC-026 | T-011, T-014 | 2 | `DivisorEndpointsTests.Put_DivisorBelowOneOrMissing_Returns400ProblemDetailsAndKeepsTheOldValue`<br>`DivisorSettings.test.tsx › shows the server error when the divisor is rejected` |
| AC-027 | T-010, T-015 | 6 | `FileEndpointsTests.List_AfterUpload_ContainsTheFileNewestFirst`<br>`OpenApiDocumentTests.OpenApiDocument_DescribesEveryPublicEndpoint`<br>`InMemoryUploadedFileStoreTests.ListNewestFirst_OrdersByUploadTimeDescending`<br>`FileUpload.test.tsx › reports a successful upload so the list can show the new entry`<br>… +2 more |
...
| AC-015 | T-009, T-010, T-011, T-013 | 9 | `FileEndpointsTests.Upload_WithoutAFile_Returns400InvalidFile`<br>`FileEndpointsTests.Download_UnknownId_Returns404FileNotFound`<br>`DivisorEndpointsTests.Put_NonIntegerDivisor_Returns400ProblemDetails`<br>`HostTests.UnknownApiRoute_ReturnsNotFoundProblemDetails`<br>… +5 more |
