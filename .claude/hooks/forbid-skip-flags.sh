...
declare -a patterns=(
  ...
  # .NET equivalents. --no-restore is deliberately NOT listed: it is a legitimate
  # fast-loop flag that skips package restore, not verification.
  '-p:SkipTests'          'Refusing to skip tests.'
  '-p:RunAnalyzers=false' 'Refusing to disable Roslyn analyzers.'
  '-p:TreatWarningsAsErrors=false' 'Refusing to downgrade warnings-as-errors.'
  '-p:EnforceCodeStyleInBuild=false' 'Refusing to skip code-style enforcement.'
  'dotnet[[:space:]]+test[^|;&]*--no-build' 'Refusing to run tests against a stale build; analyzers run at build time.'
)
